...
| **ai** | open-webui (Deploy, on NUC) + selector-less `ollama` Service → .110 + **litellm** router (Deploy, on zullx) | LLM chat; LiteLLM routes to local Ollama + Anthropic Claude (one endpoint, NodePort 32500). Local aliases by role: `local-fast` (gemma4:12b), `local-chat` (qwen3.6:35b-a3b), `local-smart` (qwen3.8:27b) |
